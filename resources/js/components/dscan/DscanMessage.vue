<template>
  <div>
    <q-btn
      ...
      round
      flat
      @click="showDscanNotes = true"
      ><q-badge v-if="messageUnreadCount && !showDscanNotes" color="red" floating>{{
        messageUnreadCount
      }}</q-badge></q-btn
    >
    <q-dialog v-model="showDscanNotes" @before-show="open()" @before-hide="close()">
      <q-card
      ...
      >
        <q-card-section class="bg-primary myCardHeader text-center">
          <div class="text-h6">Notes for Dscan <span></span></div>
        </q-card-section>
...
onMounted(async () => {});
let mainText = $ref();
let showDscanNotes = $ref(false);

const remove = async (id) => {
  await axios({
    method: "delete",
    url: "/api/dscanmessage/" + id,
...
const sendMessage = async () => {
  ...
  await axios({
    method: "put",
    url: "/api/dscanmessage/" + store.dScan.id,
    withCredentials: true,
    data: data,
    headers: {
      Accept: "application/json",
...
let open = async () => {
  store.dScanChatWindowId = store.dScan.id;
};

let close = async () => {
  mainText = null;
  store.dScanChatWindowId = null;
};
...
let messageUnreadCount = $computed(() => {
  return store.getDscanUnreadMessageCount(store.dScan.id);
});
