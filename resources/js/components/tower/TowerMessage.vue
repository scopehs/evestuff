<template>
  <div>
    <q-btn
      ...
      round
      flat
      @click="showTowerNotes = true"
      ><q-badge v-if="messageUnreadCount && !showTowerNotes" color="red" floating>{{
        messageUnreadCount
      }}</q-badge></q-btn
    >
    <q-dialog v-model="showTowerNotes" @before-show="open()" @before-hide="close()">
      <q-card
      ...
      >
        <q-card-section class="bg-primary myCardHeader text-center">
          <div class="text-h6">Notes for {{ props.tower.moon.name }}</div>
        </q-card-section>
...
onMounted(async () => {});
let mainText = $ref();
let showTowerNotes = $ref(false);

const remove = async (id) => {
  await axios({
    method: "delete",
    url: "/api/towermessage/" + id,
...
const sendMessage = async () => {
  ...
  await axios({
    method: "put",
    url: "/api/towermessage/" + props.tower.id,
    withCredentials: true,
    data: data,
    headers: {
      Accept: "application/json",
...
let open = async () => {
  store.towerChatWindowId = props.tower.id;
};

let close = async () => {
  mainText = null;
  store.towerChatWindowId = null;
};
...
let messageUnreadCount = $computed(() => {
  return store.getTowerUnreadMessageCount(props.tower.id);
});
...
let messages = $computed(() => {
  return store.getTowerMessages(props.tower.id);
});
